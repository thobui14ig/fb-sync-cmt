...
export class GetCommentPublicUseCase {
    ...
    async getCmtPublic(postId: string, isCheckInfoLink: boolean = false, link?: LinkEntity): Promise<IGetCmtPublicResponse | null> {
        ...
        try {
            ...
            if (postId === '1368686287816255') console.log("🚀 ~ GetCommentPublicUseCase ~ getCmtPublic ~ duration:", duration, delay?.timeRemoveProxySlow, proxy?.proxyAddress)
            ...
            // if (postId === '1368686287816255') console.log(dataComment)
            if (dataComment) {
